...
{
  int i, j;
  // Left boundary (global left edge)
  if (istglob == 0)
  {
    for (j = 0; j < ny; j++)
    {
  ...
  // Right boundary (global right edge)
  if (ienglob == nxglob - 1)
  {
    for (j = 0; j < ny; j++)
    {
  ...
  // Bottom boundary (global bottom edge)
  if (jstglob == 0)
  {
    for (i = 0; i < nx; i++)
    {
...
void set_initial_condition(int nx, int ny, int istglob, int ienglob, int jstglob, int jenglob, int nxglob, int nyglob, double *x, double *y, double **T, double dx, double dy)
{
  ...
  for (i = 0; i < nx; i++)
    for (j = 0; j < ny; j++)
    {
      T[i][j] = 0.25 * (tanh((x[i] - 0.4) / (del * dx)) - tanh((x[i] - 0.6) / (del * dx))) * (tanh((y[j] - 0.4) / (del * dy)) - tanh((y[j] - 0.6) / (del * dy)));
    }
...
int main(int argc, char **argv)
{
  ...
  double start_time, end_time, total_time, time_per_step;
